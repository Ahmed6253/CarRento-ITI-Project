<template>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
    <div class="flex custom-shadow m-4 lg:h-[680px] rounded-2xl mx-auto lg:w-[1064px] w-[364px]  bg-white ">
      ...
      <div v-if="signup && !OwnerorUser && role == 'user'" class="signUp lg:w-6/12 w-full p-4 ">
        ...
        <div class="flex flex-col lg:ml-9 mx-auto">
          <label class="text-base pt-3 text-primary_color font-medium pb-1" for="">Name</label>
          <input class="py-3 pl-4 text-base rounded-lg border-border_color border-[1px]" type="text"
            placeholder="Enter your name" />
          <label class="text-base pt-3 text-primary_color font-medium pb-1" for="">Email address</label>
          <input class="py-3 pl-4 text-base rounded-lg border-border_color border-[1px]" type="email"
            placeholder="Enter your Email address" />
          <label class="text-base pt-3 text-primary_color font-medium pb-1" for="">Password</label>
          <input class="py-3 pl-4 text-base rounded-lg border-border_color border-[1px]" type="text"
            placeholder="Enter your password" />
          <label class="text-base pt-3 text-primary_color font-medium pb-1" for="">Confirm password</label>
          <input class="py-3 pl-4 text-base rounded-lg border-border_color border-[1px]" type="text"
        ...
        <div class="lg:ml-9 mx-auto flex pt-3 justify-center">
          <p class="text-xl">
            Have an account?
            <button class="text-primary_color text-xl underline font-medium" @click="signup = false">Sign In</button>
          </p>
        </div>
      </div>
      ...
      <div v-if="!signup && !OwnerorUser" class="login lg:w-6/12 w-full px-4 lg:pt-24 py-12 ">
        <div class="flex gap-3 lg:ml-9 justify-start items-center">
          <img src="../imagesNavfoot/arrow_back.svg" alt="" @click="closeModal" class="pt-2" />
        ...
        <div class="lg:ml-9 mx-auto flex pt-3 justify-center">
          <p class="text-xl">
            ...
            <button class="text-primary_color text-xl underline font-medium" @click="checkrole">Sign up</button>
          </p>
        </div>
      </div>
    </div>
