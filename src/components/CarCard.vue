  ...
  >
    ...
    >
      <span
        ...
        :key="index"
        class="flex gap-1 text-[11px] text-center rounded-md w-fit px-1"
      >
        <img :src="getImagePath(prop)" alt="" />
        <p class="mt-0.5">{{ prop }}</p></span
      >
    </div>
...
export default {
  ...
  methods: {
    getImagePath(prop) {
      try {
        return require(`../assets/home-images/icons/${prop}.svg`);
      } catch (e) {
